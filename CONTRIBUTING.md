# Contributing to the SpecLynx API Language Service

Thank you for considering contributing to the SpecLynx API Language Service!
This document outlines the process for contributing to the SpecLynx API Language Service repository.
...
3.  **Code & Test:** Write your code and add tests to cover your changes. Make sure the existing test suite passes.
  *   See the [Development](#development) section below for setup instructions, testing commands, and development workflows.
...
are what the repository builds and tests on.

### Setting up

```sh
 $ git clone https://github.com/speclynx/api-languageservice.git
 $ cd api-languageservice
 $ npm i
 $ npm run build
```
...
Some npm scripts run in parallel, with a default maximum parallelization of `2`, because `CI`
runs on GitHub-hosted runners with [2-core CPUs](https://docs.github.com/en/actions/using-github-hosted-runners/about-github-hosted-runners#supported-runners-and-hardware-resources).
On a machine with more cores, set `CPU_CORES` to speed the build up:
...
| `npm run lint` / `npm run lint:fix` | ESLint over the repository |
...
`dist/` holds the UMD browser bundle and the `THIRD-PARTY-NOTICES.txt` that lists the licences of
everything the bundle embeds. `types/` holds the TypeScript declarations, both per-file and as
the rollup the package's root `types` entry points at.

### Using the monorepo as a local dev dependency

Run these inside the monorepo to link its packages into your global `node_modules`:
...
Then, from inside the dependent project, link the packages you need — all of them in a single
`npm link` invocation, because of how `npm link` works internally:
...
`node_modules` without any version of the package installed at all.

Unlinking from the global `node_modules` is not necessary, but `npm run unlink` inside the
monorepo does it.
...
-   **[🐛 Bug Report](https://github.com/speclynx/api-languageservice/issues/new?assignees=&labels=bug&template=Bug_report.md&title=%5BBug%5D+)**
-   **[✨ Feature Request](https://github.com/speclynx/api-languageservice/issues/new?assignees=&labels=enhancement&template=Feature_request.md&title=%5BFeature%5D+)**
...
Thank you for helping build and improve the SpecLynx API Language Service!
