...
describe('converter', function () {
  context('strategies', function () {
    context('openapi-3-1-to-openapi-3-0-3', function () {
      context('webhooks', function () {
        const fixturePath = path.join(__dirname, 'fixtures', 'webhooks.json');
        ...
        specify('should remove OpenAPI.webhooks field', async function () {
        ...
        specify('should create WARNING annotation', async function () {
          ...
          const annotation = annotations.find((a: AnnotationElement) => a.code?.equals('webhooks'));
