...
This document provides detailed documentation for all Arazzo linting rules through Phase 5.

## Overview

The rules provide structural validation, best-practice guidance, uniqueness enforcement, and cross-reference resolution for Arazzo 1.0.1 documents. They verify conformance to the specification in terms of required fields, field types, allowed values, naming patterns, structural constraints, runtime expression syntax, and referential integrity. The rules are organized by the Arazzo specification objects they target.
...
A cross-cutting rule validates that `<script>` tags must not appear in any `description` or `title` field across all Arazzo elements (error severity).

## Info Object

The Info Object provides metadata about the Arazzo document. The `title` and `version` fields are required. The optional `summary` and `description` fields, when present, must be strings. All four fields undergo type validation. Specification extensions are allowed. The `description` field should be present (warning) and the `summary` field is recommended (hint).
...
A Workflow describes a sequence of steps. The `workflowId` and `steps` fields are required. The `workflowId` must match `[A-Za-z0-9_\-]+`, must be unique across all workflows in the document, and the `steps` array must contain at least one entry. The `description` field should be present (warning) and the `summary` field is recommended (hint). Optional fields include `inputs` (a JSON Schema object), `dependsOn` (array of strings where each entry must be unique and must reference an existing workflow's workflowId), `successActions` and `failureActions` (arrays of Success/Failure Action or Reusable Objects), `outputs` (an object whose keys must match `[a-zA-Z0-9.\-_]+`, whose output names must be unique, and whose values must be valid Arazzo Runtime Expressions), and `parameters` (array of Parameter or Reusable Objects). Step IDs within a workflow must be unique.

## Step Object

A Step represents a single operation within a workflow. The `stepId` is required and must match `[A-Za-z0-9_\-]+`. The `description` field should be present (warning). The step should reference an operation or workflow via `operationId`, `operationPath`, or `workflowId` (all string types when present). These three fields are mutually exclusive: specifying `operationId` means `operationPath` and `workflowId` must be absent, and likewise for each of the other two fields. Using `operationPath` produces a hint suggesting `operationId` is preferred. When `workflowId` is specified, it must reference an existing workflow in the document (external `$sourceDescriptions` references are allowed). Optional fields include `parameters`, `requestBody` (a Request Body Object), `successCriteria` (array of Criterion Objects), `onSuccess` and `onFailure` (arrays of action/reusable objects), and `outputs` (an object whose keys must match `[a-zA-Z0-9.\-_]+`, whose output names must be unique, and whose values must be valid Arazzo Runtime Expressions).

## Parameter Object

A Parameter specifies a value to pass to an operation or workflow. The `name` and `value` fields are required. When present, the `in` field must be one of `"path"`, `"query"`, `"header"`, or `"cookie"`. Parameters within a step must be unique by their `name` and `in` combination. If the `value` is a string starting with `$`, it must be a valid Arazzo Runtime Expression. Only name, in, and value fields are allowed alongside extensions.

## Success Action Object

A Success Action defines behavior on step success. Both `name` and `type` are required. The `type` must be either `"end"` or `"goto"`. Success action names within a step must be unique. Optional fields include `workflowId` and `stepId` (both strings), which are mutually exclusive with each other. These fields are only relevant when the `type` is `"goto"`. When `workflowId` is specified, it must reference an existing workflow. When `stepId` is specified, it must reference an existing step within the same workflow. The optional `criteria` field is an array of Criterion Objects.

## Failure Action Object

A Failure Action defines behavior on step failure. Both `name` and `type` are required. The `type` must be one of `"end"`, `"goto"`, or `"retry"`. Failure action names within a step must be unique. The `workflowId` and `stepId` fields are mutually exclusive with each other and are only relevant when the `type` is `"goto"` or `"retry"`. When `workflowId` is specified, it must reference an existing workflow. When `stepId` is specified, it must reference an existing step within the same workflow. The `retryAfter` (a non-negative number representing seconds) and `retryLimit` (a non-negative integer) fields only apply when the `type` is `"retry"`. The optional `criteria` field is an array of Criterion Objects.
...
A Criterion defines a condition for evaluating success. The `condition` field is required and must be a string. When the `type` is `"regex"` (or a CriterionExpressionType with type `"regex"`), the `condition` must be a valid regular expression. The optional `context` must be a string and, when present, must be a valid Arazzo Runtime Expression. When the `type` field is specified, the `context` field MUST also be provided. The `type` field, when present as a string, must be one of `"simple"`, `"regex"`, `"jsonpath"`, or `"xpath"`. It may also be a Criterion Expression Type Object.
...
The Request Body provides content for operations. The optional `contentType` must be a string and should be a valid MIME type (e.g. `application/json`). The `payload` field accepts any value. The `replacements` field, when present, must be an array of Payload Replacement Objects.
...
## Phase 5: Spectral Arazzo Rules Import

Phase 5 imported validation rules inspired by the Spectral Arazzo ruleset, organized into three batches covering best-practice guidance, uniqueness validation, and cross-reference/semantic validation.

Batch 1 added 7 best-practice rules: `description` should be present on Info, Workflow, and Step objects (warning severity); `summary` is recommended on Info and Workflow objects (hint severity); `operationPath` produces a hint suggesting `operationId` is preferred; and `<script>` tags are forbidden in all `description` and `title` fields (error severity).

Batch 2 added 8 uniqueness rules: `workflowId` must be unique across all workflows; `stepId` must be unique within each workflow; output names must be unique per workflow and per step; parameters must be unique by `name`+`in` combination per step; success and failure action names must be unique per step; and `dependsOn` array entries must be unique. These rules use the `apilintPropertyUniqueValue`, `apilintSiblingUniqueValue`, `apilintSiblingUniqueCompositeValue`, `apilintArrayUniqueValues`, and `apilintNoDuplicateKeys` linter functions.

Batch 3 added 12 cross-reference and semantic validation rules using `@swaggerexpert/arazzo-runtime-expression` (v2.0.3) for runtime expression validation per the Arazzo ABNF grammar. Workflow and step output values must be valid Runtime Expressions. The `dependsOn` array entries must resolve to existing workflow IDs. Step `workflowId` must reference an existing workflow (external `$sourceDescriptions` references are skipped). Success and failure action `workflowId` and `stepId` fields must reference existing workflows and steps respectively (stepId resolution is scoped to the containing workflow). Criterion `context` must be a valid Runtime Expression, and `condition` must be a valid regular expression when `type` is `"regex"`. Request body `contentType` should be a valid MIME type. Parameter values starting with `$` must be valid Runtime Expressions.

Existing test fixtures were updated to use correct Runtime Expression syntax (`$response.body#/name` JSON Pointer form instead of the invalid `$response.body.name` dot notation).
