# Addon to Phase 4: Update JSON Shema testing to include JSON Schema 2020-12 related tests
...
The goal is to update or add the tests (if not already ok) related to: "The Arazzo parser does not create typed Schema/JSONSchema elements for inline YAML objects within nested schema fields (e.g., objects under `items`, `contains`, `if`, `then`, `else`, `not`, `propertyNames`, `additionalItems`)." This means:
